...
| Negative point estimates | 99.3% of observations |
| CATE × GDP per capita | r = −0.16 |
| CATE × Institutional Quality | r = −0.09 |
| DCI × Institution p-value | **< 0.001** (OLS; standard SE) |
...
```bash
python -m scripts.solve_wdi_v4_expanded_zip  # Download 60 vars
python -m scripts.impute_mice                # Fold-safe MICE Imputation
```

**Phase 2: Heterogeneity Verification**
```bash
python -m scripts.phase1_mvp_check           # ⭐ Interaction term test
```

**Phase 3: Causal Forest Analysis (Main)**
```bash
python -m scripts.phase2_causal_forest       # ⭐ Train Causal Forest (2000 trees)
```

**Phase 4: Visualization**
```bash
MPLBACKEND=Agg MPLCONFIGDIR=/tmp python -m scripts.phase3_visualizations  # ⭐ Generate figures
```
